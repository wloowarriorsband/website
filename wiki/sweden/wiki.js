...
const imgs = document.getElementsByClassName("sweden-carousel-img");
...
const imgToIdx = {};
// set up img to idx mappings
for (let idx = 0; idx < imgCount; ++idx) {
    const src = imgs[idx].children[0].getAttribute("src");
    const imgName = src.split("/").at(-1);
    imgToIdx[imgName] = idx;
    console.log(imgName, idx);
}


const changeImg = () => {
    idxBar.innerText = `${"○".repeat(idx)}●${"○".repeat(imgCount - 1 - idx)}`;
    for (let i = 0; i < imgCount; ++i) { // un-render all images except for selected one
        if (i !== idx) {
            imgs[i].style.zIndex = -999;
            imgs[i].style.opacity = 0; // fade out
        } else {
            imgs[i].style.zIndex = 999;
            imgs[i].style.opacity = 1;
            imgs[i].style.opacity = 1; // repeat so it skips fading in
        }
    }
}
...
const resetCycle = () => {
    ...
    for (let id = maxTimeoutId; id >= 0; --id) {
        clearTimeout(id);
    }
    setTimeout(updateCarousel, CYCLE_DURATION * 1000);
}

const changeImgIdx = (idxChange) => {
    idxChange = ((idxChange % imgCount) + imgCount) % imgCount;
    idx = (idx + idxChange) % imgCount;
    changeImg();
    resetCycle();
}

const setImg = (imgName) => {
    idx = imgToIdx[imgName];
    changeImg();
    resetCycle();
}

const updateCarousel = () => { // 3 s cycle
    changeImgIdx(1);
    changeImg();
    setTimeout(updateCarousel, CYCLE_DURATION * 1000);
}
